...
public class ETopUpSMSVER implements Job
{
	...
	public void eTop()
	{
		...
		String CC_NUMBER = " ";//"For assistance call Customer Support 84455516 [11am-11pm]";//"84455516";
		String modem_id="2";
		...
		try
		{
			...
			if(count > 0)
			{
				...
				qry ="Select id,mobile,REPLACE(REPLACE(msg, CHR(13), ' '), CHR(10), ' '),NVL(supplier_id,'-1') from sms_inbox@skydb_muci_link where modem_id = "+modem_id+" and status = 'N'";
				System.out.println(qry);
				rs = stmt.executeQuery(qry);
				while(rs.next())
				{
...
/***************************************************************************************************************************/
						qry ="Update sms_inbox@skydb_muci_link set destination = '"+destPhnNo+"',pin='"+pin+"',txn_processed='P' where modem_id = "+modem_id+" and status = 'P' and id="+aVal[0]+"";
						System.out.println(qry);
						stmt.executeUpdate(qry);
...
/********************* Check Ignore From and To Numbers **********************************/
						...
						if(ignoreFound == true)
						{
							...
							msg = "Sorry Invalid destination number, "+CC_NUMBER+"";
						...
						if(pinFound == false)
						{
							...
							retry_count=0;
							msg = "Sorry your pin is not valid. "+CC_NUMBER+"";
							...
							while(rs.next())
							{
								if(rs.getInt(1) >0)
								{
									msg = "Sorry this pin is already used. "+CC_NUMBER+"";
								}
						...
						if(destPhnNo ==null)
						{
							...
							msg="Sorry Invalid mobile number. "+CC_NUMBER+"";
							System.out.print("Sorry Invalid mobile number. "+CC_NUMBER+"");
...
/******************************** Hard Coded Supplier ID ENDS ***************************************************************/
							...
							if(supplier_id != null && supplier_id.equals("23"))
							{
								...
								if(india_op_id != null && india_op_id.equals("-1"))
								{
									...
									msg="AAARECHARGE:Sorry Operator not supported. "+CC_NUMBER+"";
									System.out.print("Sorry Operator not supported. "+CC_NUMBER+"");
				...
				/********** CHECK AMOUNT - SERIES - COMMISSION ******************************/
						...
						if(Ramt != null && Ramt.length()>0)
						{
							...
							{
								System.out.println("<font color=\"RED\" size=4>Invalid Tariff. "+CC_NUMBER+"</font>");
								qry ="Update sms_inbox@skydb_muci_link set status = 'Y',process_time=sysdate where modem_id = "+modem_id+" and status = 'P' and id="+aVal[0]+"";
								System.out.println(qry);
								stmt.executeUpdate(qry);

								msg = "Invalid Tariff. "+CC_NUMBER+"";
								qry = " Insert into sms_outbox@skydb_muci_link (id, entry_date,mobile,msg,inbox_id,modem_id) values "+
									  " (sms_outbox_sq.nextval@skydb_muci_link,sysdate, '"+aVal[1]+"','"+msg+"',"+aVal[0]+","+modem_id+")";
								System.out.println(qry);
								stmt.executeUpdate(qry);
...
/******************************** Hard Coded Supplier ID ENDS ***************************************************************/
					...
					/************************ Split amount from one21BD/eGen ENDS *************************/
							...
							qry = "Update topup_request set supp_id="+supplier_id+",egen_id="+clientTranId+" where msgid='"+areaName+"'";
							System.out.println(qry);
							stmt.executeUpdate(qry);
