...
public class ETopUpSMSACC implements Job
{
				...
				if (aList!= null && aList.size() >0)
				{
					...
					for(int h=0;h<aList.size();h++)
					{
						...
						if(ignoreFound == true)
						{
							...
							msg = "Venus Eload:Sorry Invalid destination number, for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
						...
						if(!meFound) // shop not found
						{
							qry ="Update sms_inbox@skydb_muci_link set status = 'Y',process_time=sysdate where modem_id = "+modem_id+" and status = 'P' and id="+aVal[0]+"";
							System.out.println(qry);
							stmt.executeUpdate(qry);

							msg = "Venus Eload:Athentication Failed, for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
					...
					if(aVal[2].indexOf("RS")>=0)
					{
						System.out.println(">>>>>>>"+aVal[2]);
						destPhnNo = aVal[2].substring(aVal[2].indexOf("RS")+2,aVal[2].indexOf("A"));
						System.out.println(">>destPhnNo>>>>>"+destPhnNo);
						keyVal = "S"+aVal[2].substring(aVal[2].indexOf("A")+1);
						System.out.println(">>keyVal>>>>>"+keyVal);
					}
					if(aVal[2].indexOf("RM")>=0)
					{
						destPhnNo = aVal[2].substring(aVal[2].indexOf("RM")+2,aVal[2].indexOf("A"));
						keyVal = "M"+aVal[2].substring(aVal[2].indexOf("A")+1);
					}
					if(aVal[2].indexOf("RH")>=0)
					{
						destPhnNo = aVal[2].substring(aVal[2].indexOf("RH")+2,aVal[2].indexOf("A"));
						keyVal = "H"+aVal[2].substring(aVal[2].indexOf("A")+1);
					}
					/************************ // Shop setting Speeddial **********************************/

					/************************ End Shop setting Speeddial **********************************/
						...
						if(destPhnNo != null && destPhnNo.equals("BAL"))
						{
								...
								msg = "Venus Eload:Your Balance is "+twoDigit.format(recvAmount)+" "+recvCurrency+". Today Sales:";
						...
						if(keyFound)
						{
							...
							}catch(Exception localException)
							{
								...
								msg = "Venus Eload:Invalid Keword/Value, for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
								qry = " Insert into sms_outbox@skydb_muci_link (id, entry_date,mobile,msg,inbox_id,modem_id) values "+
									  " (sms_outbox_sq.nextval@skydb_muci_link,sysdate, '"+aVal[1]+"','"+msg+"',"+aVal[0]+","+modem_id+")";
								System.out.println(qry);
								stmt.executeUpdate(qry);
						...
						if(theType != null && theType.equals("TT")) // BIKASH
						{
							...
							if(countryCode != null && countryCode.equals("880"))
							{
								...
								if(destPhnNo!=null && destPhnNo.length() != 13)
								{
									...
									msg="Venus Eload:Sorry Invalid mobile number for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
							...
							if(amount != null && amount.length()>0)
							{
								...
								if(theCount > 1)
								{
									msg="Venus Eload:Sorry Invalid Rate Configuration. for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
							...
							if(amount != null && amount.length()>0)
							{
								...
								{
									msg="Venus Eload:Sorry Invalid Amount To Recharge. for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
							...
							{
									msg="Venus Eload:orry Invalid Amount To Recharge. for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
									System.out.print("Sorry Amount To Recharge !! for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]");
							...
							if(recvAmount*(-1)+Double.parseDouble(localAmount)*(1-Double.parseDouble(smsComm)/100)+ Double.parseDouble(serviceCharge)> crLimit)
							{
									...
									msg = "Venus Eload:Sorry, You do not have enough balance. Your balance:"+twoDigit.format(recvAmount)+". For assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
									qry = " Insert into sms_outbox@skydb_muci_link (id, entry_date,mobile,msg,inbox_id,modem_id) values "+
										  " (sms_outbox_sq.nextval@skydb_muci_link,sysdate, '"+aVal[1]+"','"+msg+"',"+aVal[0]+","+modem_id+")";
									System.out.println(qry);
									stmt.executeUpdate(qry);
							...
							if(succ.indexOf("Successful")<0)
							{
								...
								msg="Venus Eload:Sorry Recharge Failed. for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
...
*/
						...
						}else if(theType != null && theType.equals("PIN")) // Third Party PIN Block
						{

							/********************* Credit Limit Check **************************/
							if(recvAmount*(-1)+val > crLimit)
							{
									qry ="Update sms_inbox@skydb_muci_link set status = 'Y',process_time=sysdate where modem_id = "+modem_id+" and status = 'P' and id="+aVal[0]+"";
									System.out.println(qry);
									stmt.executeUpdate(qry);
							...
							qry = "Select name from tp_package_config where id="+tpPackId;
							rs = stmt.executeQuery(qry);
							while(rs.next())
							{
							...
							qry="select func_tp_pin("+me_id+","+amount+",'"+prefix+"') from dual";
							...
							try
							{
								...
								if(theRet != null && theRet.indexOf("F:")>0)
								{
									qry ="Update sms_inbox@skydb_muci_link set status = 'Y',process_time=sysdate where modem_id = "+modem_id+" and status = 'P' and id="+aVal[0]+"";
									System.out.println(qry);
									stmt.executeUpdate(qry);

									msg = "Venus Eload:Your Request Failed. for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
									qry = " Insert into sms_outbox@skydb_muci_link (id, entry_date,mobile,msg,inbox_id,modem_id) values "+
										  " (sms_outbox_sq.nextval@skydb_muci_link,sysdate, '"+aVal[1]+"','"+msg+"',"+aVal[0]+","+modem_id+")";
									System.out.println(qry);
									stmt.executeUpdate(qry);
								...
								}else
								{
									...
									qry = "Select serial_no, pin, (Select instruction from tp_package_config where id = tp_pakg_id) from tp_inventory where id = "+theRet;
									System.out.println(qry);
									rs=stmt.executeQuery(qry);
									...
									while(rs!=null && rs.next())
									{
										...
										pin = rs.getString(2);
										instruction = rs.getString(3); if(instruction == null || instruction.equals("null")) instruction = "";
										theResponse = "Serial:"+serial+":PIN:"+pin+":"+instruction+"";
									}
									qry = "select flx_id.nextval from dual";
									System.out.println(qry);
									rs= stmt.executeQuery(qry);
									...
									qry = "insert into topup_request(id, msgid, src_ip, originator, prefix,dst_no, amount, history_id, topup_request_time,topup_process_time,processed,orig_amount,col1,me_id,telco,response,inbox_id) values (topup_sq.nextval,'"+areaName+"','10.1.1.2','"+aVal[1]+"','01','"+destPhnNo+"', "+amount+",-1 ,sysdate,sysdate,'Y',"+amount+",'J8BugdDFB',"+me_id+",'"+packName+"'||'-SG','"+theResponse+"',"+aVal[0]+")";
									System.out.println(qry);
									stmt.executeUpdate(qry);
									...
									msg = "Venus Eload:"+destPhnNo+":$"+amount+":"+theResponse+".Bal:"+twoDigit.format(recvAmount);
									qry = " Insert into sms_outbox@skydb_muci_link (id, entry_date,mobile,msg,inbox_id,modem_id) values "+
										  " (sms_outbox_sq.nextval@skydb_muci_link,sysdate, '"+aVal[1]+"','"+msg+"',"+aVal[0]+","+modem_id+")";
									System.out.println(qry);
									stmt.executeUpdate(qry);
						...
						}else if(theType != null && theType.equals("FLX"))// Flexi Block
						{
							...
							long destPhnNo_long=0;
							try
							{
								destPhnNo_long = Long.parseLong(destPhnNo);
							}catch(Exception localException)
							{
									msg="Venus Eload:Sorry Invalid mobile number for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
									System.out.print("Sorry Invalid mobile number for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]");
							...
							if(timeDiff > 0)
							{
								...
								msg = "Venus Eload:This request can not be processed now. Please try after "+timeDiff+" minute";
							...
							if(supplier_id == null || supplier_id.equals("-1"))
							{
									msg="Venus Eload:Sorry No Supplier. for assistance call Customer Support "+CC_NUMBER+" [11am-11pm]";
							...
							if(ret == 0)
							{
								...
								qry = "Update topup_request set me_id ="+me_id+", supp_id="+supplier_id+",shop_comm="+smsComm+",series_id="+seriesId+", inbox_id= "+aVal[0]+",egen_id="+clientTranId+",prepost="+prepost+" where msgid='"+areaName+"'";
								System.out.println(qry);
								stmt.executeUpdate(qry);
								...
								if(supplier_id != null && !supplier_id.equals("12") && !supplier_id.equals("13") && !supplier_id.equals("14"))
								{
									...
									}else
									{
										...
										if(urlStr != null && urlStr.indexOf("119.81.14.114")>=0)
										{
										...
										if(txnid != null && txnid.equals("-1"))
										{
											tmpStr = "N";

										}else if(txnid != null && txnid.length()>= 1)
										{
											tmpStr = "Y";
										}
...
//												msg = "Recharge to "+destPhnNo+" successfully completed by "+amountToRecharge+" "+currencyName+". Transaction no-"+aVal[0]+"";
												msg = "Success! Bangladesh: "+currencyName+" "+amountToRecharge+" to "+destPhnNo+". ";
											}else
											{
...
//												msg = "Recharge to "+telcoName+" number "+destPhnNo+" successfully completed by "+amountToRecharge+" "+currencyName+". Transaction no-"+txnid+"";
												msg = "Success! "+telcoName+": $"+amountToRecharge+" to "+tmpDestPhone+". ";//TXID: "+aVal[0]+"";
											}
										...
										qry = "Update topup_request set processed='"+tmpStr+"',transaction_id = '"+txnid+"',topup_process_time = sysdate, response='"+remarks+"', inbox_id= "+aVal[0]+",reply_sms='"+msg+"' where msgid = '"+areaName+"'";
										...
										msg = ""+msg+" Your Bal now is $ "+twoDigit.format(recvAmount);
										qry = "Update topup_request set balance='"+twoDigit.format(recvAmount)+"',shop_mobile='"+aVal[1]+"' where msgid = '"+areaName+"'";
										System.out.println(qry);
										stmt.executeUpdate(qry);
								...
								{
									qry = "Update topup_request set processed='Y',transaction_id = id,topup_process_time = sysdate, response='REQUEST SUBMITTED' , inbox_id= "+aVal[0]+" where msgid = '"+areaName+"'";
...
//									msg = "Recharge to "+telcoName+" number "+destPhnNo+" successfully completed by "+amountToRecharge+" "+currencyName+". Transaction no-"+aVal[0]+"";
									msg = "Success! "+telcoName+": $"+amountToRecharge+" to "+tmpDestPhone+". ";//TXID: "+aVal[0]+"";


//									msg = "Venus Eload:"+msg+" Balance: "+twoDigit.format(recvAmount);
									msg = ""+msg+" Your Bal now is $ "+twoDigit.format(recvAmount);
									qry = "Update topup_request set reply_sms='"+msg+"',balance='"+twoDigit.format(recvAmount)+"',shop_mobile='"+aVal[1]+"' where msgid = '"+areaName+"'";
									System.out.println(qry);
									stmt.executeUpdate(qry);
									...
									stmt.executeUpdate(qry);
									*/
									qry ="Update sms_inbox@skydb_muci_link set txn_processed='Y' where modem_id = "+modem_id+" and status = 'P' and id="+aVal[0]+"";
									System.out.println(qry);
									stmt.executeUpdate(qry);
